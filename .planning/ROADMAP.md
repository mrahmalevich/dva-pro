...
**Created:** 2026-04-26
**Last reordered:** 2026-04-27 — Phase 6 (Inventory Pipeline) promoted to Phase 1 with reduced file-output scope; Compliance & Infra demoted to Phase 3; Phases 2–4 shifted down by one. See `## Reorder Log` at bottom for rationale.
...
- [ ] **Phase 6: Lead Flow End-to-End (Quiz → PDF → Email)** — `POST /api/public/leads`, pg-boss pipeline, Cyrillic PDF, Unisender Go отправка с landed-cost stub
- [ ] **Phase 7: Admin Auth, RBAC & Real Admin Panel** — Better-Auth с ролями founder/sales_rep, audit_log, LeadsAdmin/CarsAdmin/SettingsAdmin поверх real DB
- [ ] **Phase 8: Public Site Polish, Multi-Market UI & Analytics** — копирайт под 6 рынков, floating widget, флаги/бейджи, Yandex Metrika + 4 цели, mobile/Yandex Browser smoke-test
- [ ] **Phase 9: Pre-Launch Checklist & Soft-Launch** — «Looks Done But Isn't» check, 2× E2E прогона, 24h мониторинг, тренинг команды
...
**Success Criteria** (what must be TRUE):
  ...
  10. README at `data/scraped/README.md` (or repo top-level docs) explains how to run, where output lands, what stubs do, and how Phase 4's future importer should consume the JSON.
...
- [x] 01-08-PLAN.md — SCHEMA.md + README.md docs (Phase 4 handoff)
...
**Complexity:** M — narrowed scope (one real scraper + 4 stubs + shared plumbing) makes this smaller than the original Phase 6 design
...
**Parallelisable with:** Phase 3 (Compliance & Infra) — no shared dependency surface; founder content collection
**UI hint:** no
**Notes on what was cut from original Phase 6:**
- No Drizzle schemas, migrations, or pg-boss queue (deferred to Phase 4)
...
- [x] 02.1-03-PLAN.md — Wave 1: Catalog within-section trim — heading-row + bottom-CTA gaps (D-02b Offender #1)
- [x] 02.1-04-PLAN.md — Wave 1: Process within-section trim — heading-margin + per-step row padding (D-02b Offender #2 — largest within-section saving)
- [x] 02.1-05-PLAN.md — Wave 1: Founders within-section trim — outer grid gap + portrait-grid → quote-block gap (D-02b Offender #3)
- [x] 02.1-06-PLAN.md — Wave 2: empirical re-tune of `DIFF_THRESHOLD` + post-densification VERIFICATION + ROADMAP/STATE close (D-01a + D-03 + D-03a + D-03b + D-04a)
...
### Phase 4: Schema, API Skeleton & Country Registry
...
**Depends on:** Phase 3 (нужны provisioned PG + image registry + GitLab CI), Phase 1 (JSON contract + drom output) — soft dependency: importer reads Phase 1 artifacts but Phase 4 schema can be designed in parallel as long as the JSON contract from Phase 1 §SC-2 is locked first
...
### Phase 5: Frontend Integration, Consent & Legal
...
**Depends on:** Phase 4 (нужны public read API + shared types)
...
**Parallelisable with:** Phase 6 lead-flow backend work (pipeline на бэке можно строить пока фронт мигрирует), founder content
**UI hint:** yes

---

### Phase 6: Lead Flow End-to-End (Quiz → PDF → Email)
...
**Depends on:** Phase 3 (DNS warm-up, S3 bucket), Phase 4 (`leads` schema + queue tables + cars from drom importer), Phase 5 (consent UI собирает согласие до сабмита)
...
**Success Criteria** (what must be TRUE):
  ...
  5. Sales-channel (founder mailbox) получает BCC каждого лид-email; matcher выдаёт ≥1 авто из admin-curated fallback / model-DB (drom data, imported in Phase 4) даже когда live inventory ещё пуст
**Plans**: TBD
**Complexity:** L
...
**Parallelisable with:** Phase 7 admin polish (но не leads-admin — она ждёт auth)
**UI hint:** yes

---

### Phase 7: Admin Auth, RBAC & Real Admin Panel
...
**Depends on:** Phase 4 (`users`, `sessions`, `audit_log` schema + drom-imported cars/models), Phase 6 (real `leads` table уже наполняется)
...
**Parallelisable with:** Phase 8 content polish
**UI hint:** yes

---

### Phase 8: Public Site Polish, Multi-Market UI & Analytics
...
**Depends on:** Phase 5 (`CrmProvider` уже на real API), Phase 4 (drom-imported cars в каталоге), Phase 7 (admin может править content live)
...
**Parallelisable with:** Phase 7 admin polish; founder content writing завершается здесь
**UI hint:** yes

---

### Phase 9: Pre-Launch Checklist & Soft-Launch
...
**Depends on:** Phase 8 (site complete) — gate, не creative work
...
- Phase 1 (drom scraper → JSON/WebP) и Phase 3 (Compliance & Infra) стартуют параллельно в неделю 1 — у них нет общей dependency surface. Phase 1 локальный, Phase 3 — Roskomnadzor 5-day window + Unisender Go 14-day warm-up в фоне.
...
- Phase 4 schema до Phase 5 frontend rewrite — иначе CrmProvider пишется против моков, а потом переписывается дважды. Phase 4 importer закрывает Phase 1 → DB переход.
- Phase 5 consent UI до того, как любая публичная форма пойдёт live в Phase 6 — 152-ФЗ irreversible.
- Phase 6 lead flow до Phase 7 (admin auth) — leads нужны до того, как LeadsAdmin может что-то показать.
- Phase 7 admin auth до того, как LeadsAdmin пойдёт live — leads = PII, нужен audit + RBAC gate.
- Phase 8 → Phase 9 — content/QA/analytics до launch checklist.

**Parallel tracks (config has parallelization=true):**
- **Phase 1 + Phase 3 в неделю 1** — независимые dependency surfaces. Phase 1 локальный, не требует Yandex Cloud; Phase 3 — pure infra/compliance.
- **Phase 01.1 + Phase 3** — also independent. Phase 01.1 is local-only (drom + dev machine); Phase 3 is pure infra/compliance.
- **Founder content** (биографии, фото, отзывы, видео-pitch) — pure content, можно начинать в Phase 1 и завершать в Phase 8; не блокирует никакую код-фазу.
- **PDF template design draft** — стартует как только schema из Phase 4 готова, готовится параллельно с Phase 5.
- **Phase 7 admin polish (CarsAdmin / FaqAdmin / ReviewsAdmin)** — может стартовать параллельно с Phase 6 backend (leads-admin ждёт auth, но cars/faq/reviews не требуют lead-flow).
- **Unisender Go warm-up + DNS reputation monitoring** — продолжаются как фоновая задача с Phase 3 до Phase 9.
...
|------|--------------|----------------|
| 1 (Apr 27 – May 3) | Phase 1 (drom scraper + stubs end-to-end) + Phase 3 (infra + Roskomnadzor подача + DNS warm-up start) | Founder content kickoff |
| 2 (May 4 – May 10) | Phase 01.1 (extend-drom-scrape-fields BMW pilot) + Phase 4 (schema + API skeleton + country registry + Phase 1 importer) | Phase 5 prep, PDF template design |
| 3 (May 11 – May 17) | Phase 5 (CrmProvider rewrite + consent + legal) + Phase 6 start (PDF pipeline) | Phase 7 admin scaffolding |
| 4 (May 18 – May 24) | Phase 6 finish (E2E lead flow на yandex.ru/mail.ru) | Phase 7 auth scaffolding |
| 5 (May 25 – May 31) | Phase 7 (auth + admin) | Phase 8 content + analytics |
| 6 (Jun 1 – Jun 7) | Phase 8 finish (mobile + Yandex Browser + Metrika) + Phase 9 (checklist + E2E + training + launch) | — |

**Risk lines:**
- If drom partner API onboarding > 1 week → fall back to polite scrape (Cheerio + 1 req/10–15s); already covered by D-05 in old Phase 6 context (carries forward).
- If Unisender Go warm-up has spam complaints in week 2 → tighten DMARC `p=quarantine` + delay first prod-email до week 3.
- Live scrapers (Encar/BeForward/Che168/Autohome) explicitly v1.x — Phase 1 ships only stubs. CONTENT-07 (≥12 авто в каталоге) covered by drom-imported models + admin-curated cars in Phase 7.
- Phase 01.1 BMW pilot run is real-network 5–7h continuous; if drom rate-limits at 5 s probe-down floor, run aborts and re-attempts; budgeted in week 2 alongside Phase 4 schema work.
...
**2026-04-27 — Phase 6 → Phase 1 promotion + scope reduction**

Original ordering had Inventory Pipeline (Encar + drom + JP/CN scrapers + Crawlee fleet + residential proxies + Yandex Object Storage rehost + pg-boss queue) as Phase 6, depending on Phases 1 (infra) and 2 (DB schema). New ordering pulls scraping forward as Phase 1 with a much smaller scope:
...
- **DB integration** — moved to new Phase 4 (formerly Phase 3) as a separate `pnpm import:scraped` job that reads Phase 1 artifacts
...
**2026-05-07 — Phase 2 inserted (redesign from screenshot); old Phases 2–8 shifted to 3–9**

Founder request: rebuild landing page from a provided design screenshot (DVApro hero / quiz / process / catalog / reviews / lead-magnet). Inserted as Phase 2 to lock the visual baseline before backend integration phases consume the API. Existing Phases 2–8 cascade-renumbered to 3–9; only `.planning/phases/01*` directories had real content (the rest were roadmap-only stubs), so the renumber was a text rewrite plus one directory rename (`09-redesign-from-screenshot/` → `02-redesign-from-screenshot/`). Phase 2 dependency rewritten to Phase 1 (catalog/list sections need inventory data; not on the new launch phase).
