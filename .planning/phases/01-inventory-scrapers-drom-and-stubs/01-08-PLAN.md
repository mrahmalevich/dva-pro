...
must_haves:
  truths:
    ...
    - "data/scraped/SCHEMA.md describes Phase 3 importer contract (upsert key = (brand_slug, model_slug, generation))"
    - "data/scraped/SCHEMA.md notes the drom = models.json vs others = cars.json terminology distinction"
    - "data/scraped/README.md documents `pnpm scrape:drom` invocation, exit codes, output dir layout, Phase 3 consume contract"
    - "data/scraped/README.md documents the Phase 3 'always re-resolve current/' rule (Pitfall 7)"
  ...
  artifacts:
    - path: "data/scraped/SCHEMA.md"
      provides: "Field-by-field documentation of models.json + Phase 3 importer contract"
      contains: "brand_slug"
    - path: "data/scraped/README.md"
      provides: "How to run + where output lands + Phase 3 consume contract + OS support note"
...
Two committed Markdown files that complete the Phase 1 → Phase 3 handoff contract:
- `data/scraped/SCHEMA.md` — exhaustive field-by-field doc of `models.json` (drom = master models). Includes a worked BMW X5 G05 example. Lists Phase 3 importer's upsert contract: `(brand_slug, model_slug, generation)` is the unique key. Also documents the future `cars.json` shape (specific listings) for the v1.x stubbed sources, even though only drom is real in Phase 1.
- `data/scraped/README.md` — runbook: how to invoke `pnpm scrape:drom`, exit codes, where output lands, the `current/` symlink contract, manual prune command, OS support, secrets policy, the Phase 3 "always re-resolve `current/` per invocation" rule (Pitfall 7).

Purpose: Phase 1's primary deliverable beyond code is the SCHEMA contract. Phase 3's importer is a near-pure JSON→SQL mapping, but only IF SCHEMA.md is precise. Plan 09's live smoke run depends on README's runbook being correct.
...
3. Phase 3 consume contract: ALWAYS re-resolve `current/` per invocation; never cache realpath.
...
```
  ...
  <acceptance_criteria>
    ...
    - File documents Phase 3 upsert key `(brand_slug, model_slug, generation)` (grep)
  ...
  <done>
SCHEMA.md authoritative documentation in place; Phase 3 importer contract published.
  </done>
</task>
...
```
  ...
  <verify>
    <automated>test -f data/scraped/README.md &amp;&amp; grep -q "pnpm scrape:drom" data/scraped/README.md &amp;&amp; grep -q "Exit codes" data/scraped/README.md &amp;&amp; grep -q "current/" data/scraped/README.md &amp;&amp; grep -q "Phase 3" data/scraped/README.md &amp;&amp; grep -q "Windows" data/scraped/README.md &amp;&amp; grep -q "NO environment variables" data/scraped/README.md &amp;&amp; grep -q "find data/scraped/drom" data/scraped/README.md &amp;&amp; ! git check-ignore -q data/scraped/README.md</automated>
  </verify>
  <acceptance_criteria>
    ...
    - File documents `current/` symlink contract + Phase 3 "always re-resolve" rule (grep "Pitfall 7" OR "re-resolve")
  ...
  <done>
README.md runbook in place; Phase 1 → Phase 3 handoff and operator workflow documented.
  </done>
</task>
...
|----------|-------------|
| Documentation → human reader | Misleading docs cause operator error or Phase 3 importer bugs |
...
| T-08-02 | Tampering | Operator follows stale prune command and deletes a run that's still being read by Phase 3 | mitigate | Prune command uses `-mtime +30` (only files older than 30 days). `current/` symlink is intact (the find pattern excludes the symlink itself — `-name "20*Z"` matches the dir name pattern, not `current`). |
...
2. SCHEMA.md mirrors all 17 ModelRecord fields and documents the Phase 3 upsert key.
...
- `data/scraped/SCHEMA.md` is the authoritative human-readable mirror of `server/scrapers/shared/types.ts`, with the BMW X5 G05 worked example and the Phase 3 upsert contract.
...
- The Phase 3 author can read these two files and write the importer without re-asking Phase 1 questions.
</success_criteria>
...
- Phase 3 handoff readiness: the 17 ModelRecord fields + upsert key + image rehost contract are all explicit.
</output>
