...
1. Reorder phases: this scraping work is the FIRST step (Phase 1). Old Phase 1 (Compliance & Infra) becomes Phase 3; everything else shifts down by one. Done — see ROADMAP.md `## Reorder Log`.
...
3. This phase scrapes data into **structured JSON files + WebP images** on disk. The JSON output must be importable to a DB later (Phase 4), once architectural decisions about the DB schema are finalized.
...
- JSON schema documentation (`data/scraped/SCHEMA.md`) precise enough that a future Phase 4 importer can map deterministically to the DB
- README explaining how to run, where output lands, what stubs do

## What's OUT of scope (vs the original Phase 6)

- ❌ Drizzle schemas (deferred to new Phase 4)
- ❌ pg-boss queue + cron schedules (deferred to new Phase 4 worker)
...
- ❌ Soft-delete via `last_seen_at` in DB (no DB; soft-delete logic moves to Phase 4 importer)
- ❌ `(source, source_id)` UNIQUE constraint enforcement at DB level (no DB; Phase 4 importer enforces it)
...
- **`server/` vs flat repo layout** — original Phase 6 assumed a `server/` tree (also a Phase 3/3 deliverable). Without DB, do we still want `server/` as the home for scraper code, or `scrapers/` at repo root, or under existing `src/`?

## Useful Context (carry-forward from prior thinking)

- **Frontend `Country` enum** at `src/crm/types.ts:1` is `'jp' | 'cn' | 'kr'` — Phase 5 (formerly 3) widens this to all 6 markets; Phase 1 scraper output should write country values matching the eventual 6-market registry.
- **Frontend `Car` interface** at `src/crm/types.ts:4` defines public fields — drom output's normalized records should populate fields that map cleanly to this shape so the future Phase 4 importer doesn't need a heavy translation layer.
...
- `data/scraped/README.md` — how to run / where output lands / how Phase 4 will consume
